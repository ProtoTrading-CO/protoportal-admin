...
export const VIRTUAL_MOTTARO_PATH_ERROR =
...
/** True when a taxonomy id path targets the virtual Mottaro tree (not writable to DB). */
...
/**
 * Map a product's primary DB labels to its virtual Mottaro category path (node ids).
 * Primary category columns are unchanged — this is the brand browse path.
 */
...
/** True when a DB row was incorrectly assigned category "Mottaro" via bulk move. */
export function isMisplacedMotarroDbCategory(row) {
  return normLabel(row?.category) === 'mottaro';
}

/** Browse path for misplaced rows that have category=Mottaro in the DB. */
function misplacedMotarroBrowsePathFromRow(row, tree) {
  if (!isMisplacedMotarroDbCategory(row)) return [];
  const mottaroRoot = findMotarroNode(tree);
  if (!mottaroRoot) return ['mottaro'];
  const tailLabels = [
    row.subcategory_one,
  ...
  if (!tailLabels.length) return ['mottaro'];
  return resolvePathByLabels(mottaroRoot, tailLabels, ['mottaro']);
}

/** Title-inferred Mottaro path, or recovery path for misplaced DB category rows. */
function effectiveMotarroBrowsePath(row, tree) {
  if (isMotarroProduct(row)) {
    const inferred = inferMotarroPathFromRow(row, tree);
    if (inferred.length) return inferred;
  }
  return misplacedMotarroBrowsePathFromRow(row, tree);
}
...
export function filterRowsByMotarroPath(rows, categoryPath, tree) {
  ...
  return rows.filter((row) => {
    const mp = effectiveMotarroBrowsePath(row, tree);
