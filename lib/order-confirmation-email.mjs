import { buildOrderNoteSections } from '../api/_order-format.js';
...
export const ORDER_CONFIRMATION_SAMPLE = {
  customerName: 'Jane Smith',
  orderNumber: 'ORD-1042',
  orderDate: '2026-07-03T10:00:00.000Z',
  items: [
    { code: 'PEN001', name: 'Ballpoint Pen Blue', qty: 12, unitPrice: 4.5 },
    { code: 'NBK024', name: 'A4 Notebook 192pp', qty: 3, unitPrice: 28.0 },
  ],
  autoNotes: 'Standard delivery.',
  userNotes: '',
  customerNotes: '',
  assignedTo: 'Victor',
  total: 138,
  hasPrices: false,
  hasPresaleInvoice: false,
  customerDetails: [
    { label: 'Customer', value: 'Jane Smith · ABC Stationers' },
    { label: 'Delivery', value: 'Courier to Johannesburg' },
  ],
};

function defaultIntroHtml(customerName) {
  return `<p style="color:#0f172a;font-size:15px;margin:0 0 8px">Hi <strong>${escapeHtml(customerName, 'there')}</strong>,</p>
    <p style="color:#334155;font-size:14px;margin:0 0 24px;line-height:1.65">
      ...
      <strong>order confirmation PDF</strong> is attached.
    </p>`;
}
...
export function buildOrderEmailHtml({
  ...
  const introBlock = String(introHtml || '').trim() || defaultIntroHtml(customerName);
...
<div style="max-width:640px;margin:24px auto;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(15,23,42,0.08)">
  ...
  <div style="padding:28px 32px;background:#ffffff">
    ${introBlock}
    ${items.some((i) => i.removed) ? '<p style="color:#dc2626;font-weight:700;font-size:13px;margin:0 0 16px">Items with stock available 0 are not included in your confirmed order.</p>' : ''}
    ${items.some((i) => !i.removed && i.originalQty != null && i.qty !== i.originalQty) ? '<p style="color:#92400e;font-weight:700;font-size:13px;margin:0 0 16px">Items marked QTY CHANGED have been adjusted from your original order.</p>' : ''}
  ...
  <div style="padding:22px 32px;background:#0f172a;text-align:center">
    <div style="font-size:13px;color:#ffffff;font-weight:700">Proto Trading · <a href="https://proto.co.za" style="color:#ffffff">proto.co.za</a></div>
...
export function buildOrderConfirmationMergeVars({ customerName, orderNumber, orderRow, to }) {
  const name = orderRow?.customers?.contact_name || orderRow?.customers?.name || customerName || 'there';
  return {
    name,
    order_number: String(orderNumber || orderRow?.order_number || '').trim(),
  };
}
