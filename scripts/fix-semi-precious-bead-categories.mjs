#!/usr/bin/env node
/**
 * Correct semi-precious stone beads misclassified as Beads > Natural.
 *
 * Usage:
 *   node scripts/fix-semi-precious-bead-categories.mjs
 *   node scripts/fix-semi-precious-bead-categories.mjs --apply
 *   node scripts/fix-semi-precious-bead-categories.mjs --apply-if-pending
 */
...
const PENDING_FILE = join(ROOT, 'data/semi-precious-fix.pending');
const PAGE = 1000;
...
if (APPLY_IF_PENDING && !existsSync(PENDING_FILE)) {
  console.log('No semi-precious-fix.pending — skipping.');
  process.exit(0);
}
...
if (!url || !key) {
  if (APPLY_IF_PENDING) {
    console.warn('Skipping semi-precious fix — missing Supabase credentials.');
    process.exit(0);
  }
...
console.log('Loading catalogue for semi-precious bead scan…');
const allRows = [
...
for (const row of allRows) {
  ...
  const sku = row.sku || '';
  if (!isSemiPreciousProduct(title, sku)) continue;

  const targetPath = inferSemiPreciousPath(title, sku);
  const resolved = resolvePathFields(tree, targetPath);
  if (!resolved) {
    console.warn(`  Invalid path for ${sku}: ${targetPath}`);
    continue;
  }

  if (fieldsMatch(row, resolved)) {
    alreadyCorrect.push(sku);
    continue;
  }
...
console.log(`\n✓ Applied ${done}/${updates.length} semi-precious category fixes.`);

if (APPLY_IF_PENDING && done > 0 && existsSync(PENDING_FILE)) {
  unlinkSync(PENDING_FILE);
  console.log('Removed data/semi-precious-fix.pending');
}
